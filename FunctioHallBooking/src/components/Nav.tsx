...
const Nav: React.FC<{ filterQuery: (variables: object) => void }> = ({
  filterQuery,
}) => {
      ...
      <nav className="navbar navbar-dark sticky-top bg-dark">
        <div className="container-fluid mx-3">
          ...
          <div className="d-flex m-1 " role="search">
            <input
              className="form-control me-2 position-relative rounded-4"
              type="search"
              value={search}
              onChange={(event) => {
              ...
              placeholder="Enter the name of Venu"
              aria-label="Search"
            />
            ...
            <section
              className={`position-absolute bg-white rounded-2 top ${display}`}
              style={{ top: "50px", width: "205px" }}
            >
              <ul className="m-0 p-0">
                {search.length > 1 &&
                  searchdata &&
                  searchdata.searchHall.map((name: any) => {
                    return (
                      <li
                        key={name.id}
                        className="list-unstyled p-1 border-bottom"
                      >
                        <Link
                          to={`/halls/${name.id}`}
          ...
          <Modal show={modal} centered>
            ...
            <Modal.Body>
              ...
              State:{" "}
              <select
                className=" form-select btn-dark m-1"
                onChange={(event) => {
              ...
              >
                ...
                {data &&
                  data.halls.map((hall: hall, index: number) => {
                ...
                {data &&
                  data.halls.map((hall: hall, index: number) => {
                    if (!uniquesearch.includes(hall.location.city)) {
                      ...
                      return (
                        <option key={index} value={hall.location.city}>
                          {hall.location.city}
                        </option>
                      );
                    }
